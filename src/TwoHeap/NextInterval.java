...
public class NextInterval {
    public static int[] findNextInterval(Interval[] intervals) {
        ...
        for (int i = 0; i < n; i++) {
            int topEnd = maxEndHeap.poll(); // let's find the next interval of the
            // interval which has the highest 'end'
...
/*
A brute force solution could be to take one interval at a time and go through all the other intervals to find the next interval. This algorithm will take O(N^2)O(N
​2
​​ ) where ‘N’ is the total number of intervals. Can we do better than that?

We can utilize the Two Heaps approach. We can push all intervals into two heaps:
one heap to sort the intervals on maximum start time (let’s call it maxStartHeap)
